...
class SaveDataValidator {
  ...
  static SaveDataValidationResult validate(SaveData saveData) {
    ...
    if (saveData.version < minimumSupportedVersion) {
      errors.add(
        'Save data version ${saveData.version} is too old and not supported',
      );
    } else if (saveData.version > currentVersion) {
      errors.add(
        'Save data version ${saveData.version} is newer than supported version $currentVersion',
      );
    } else if (saveData.version < currentVersion) {
      needsMigration = true;
      targetVersion = currentVersion;
      warnings.add(
        'Save data version ${saveData.version} needs migration to version $currentVersion',
      );
    }
  ...
  static void _validateGridData(
    ...
    if (tiles.length != Grid.size) {
      errors.add(
        'Grid tiles array has wrong length: expected ${Grid.size}, got ${tiles.length}',
      );
      return;
    }
    ...
    for (int x = 0; x < tiles.length; x++) {
      ...
      if (row.length != Grid.size) {
        errors.add(
          'Grid row $x has wrong length: expected ${Grid.size}, got ${row.length}',
        );
        continue;
      }
  ...
  static void _validateGlobalState(
    ...
    if (globalState.containsKey('currentTick')) {
      ...
      if (currentTick is! int || currentTick < 0) {
        warnings.add(
          'Global state currentTick should be a non-negative integer',
        );
      }
    }
    ...
    if (globalState.containsKey('simulationTime')) {
      ...
      if (simulationTime is! num || simulationTime < 0) {
        warnings.add(
          'Global state simulationTime should be a non-negative number',
        );
      }
    }
    ...
    for (final entry in globalState.entries) {
      if (!_isJsonSerializable(entry.value)) {
        errors.add(
          'Global state key "${entry.key}" contains non-serializable value',
        );
      }
    }
  }
