...
class GameEventBus {
  ...
  static const int _MAX_HISTORY = 1000;
  ...
  void publish<T extends GameEvent>(T event) {
    ...
    if (_eventHistory.length > _MAX_HISTORY) {
  ...
  StreamSubscription<T> subscribe<T extends GameEvent>(
    ...
    final subscription = _eventBus.on<T>().where(where ?? (_) => true).listen(onEvent);
  ...
  List<StreamSubscription> subscribeMultiple<T extends GameEvent>(
    ...
    for (final eventType in eventTypes) {
      final subscription = _eventBus.on<T>().where((event) {
  ...
  String getDebugInfo() {
    final totalEvents = _eventCounts.values.fold(0, (sum, count) => sum + count);
    ...
    for (int i = 0; i < topEvents.length && i < 5; i++) {
      ...
      buffer.writeln('    ${entry.key.toString().split('.').last}: ${entry.value}');
    }
...
class EventSubscriptionManager {
  ...
  List<StreamSubscription> subscribeMultiple<T extends GameEvent>(
    ...
    final subscriptions = _eventBus.subscribeMultiple<T>(eventTypes, onEvent, where: where);
...
extension GameEventBusExtensions on GameEventBus {
  ...
  void publishSimulationError(Object error, String context, [StackTrace? stackTrace]) {
    publish(SimulationErrorEvent(
      error: error,
      context: context,
  ...
  void publishRenderError(Object error, String context, [StackTrace? stackTrace]) {
    publish(RenderErrorEvent(
      error: error,
      context: context,
      stackTrace: stackTrace,
      timestamp: DateTime.now(),
