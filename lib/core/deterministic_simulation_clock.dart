...
class DeterministicSimulationClock implements SimulationClock {
  static const List<double> _speedMultipliers = [0.5, 1.0, 2.0, 3.0];

  Timer? _timer;
  final StreamController<TickEvent> _tickController =
      StreamController<TickEvent>.broadcast();
  ...
  void setSpeedMultiplier(double multiplier) {
    ...
    _speedMultiplier = _speedMultipliers.reduce(
      (a, b) => (a - multiplier).abs() < (b - multiplier).abs() ? a : b,
    );
  ...
  void _scheduleNextTick() {
    ...
    final adjustedDuration = Duration(
      milliseconds:
          (SimulationClock.tickDuration.inMilliseconds / _speedMultiplier)
              .round(),
    );
  ...
  void _executeTick() {
    ...
    final simulationTime = _currentTick / SimulationClock.ticksPerSecond;
  ...
  Duration get simulationTime => Duration(
    milliseconds: (_currentTick * SimulationClock.tickDuration.inMilliseconds)
        .round(),
  );
